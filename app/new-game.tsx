...
export default function NewGameScreen() {
  ...
  const defaultName = useMemo(() => {
    ...
    return `${hour12}${ampm} ${dayName}${scriptName ? ` ${scriptName}` : ''} ${monthName} ${now.getDate()}`;
  }, [
    selectedScript,
    scripts.find,
  ]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View
          style={[
            isWide && styles.twoColumn,
          ]}
        >
          {/* Left Column: Game Name + Script */}
          <View
            style={[
              isWide && styles.columnLeft,
            ]}
          >
            {/* Game Name */}
            ...
            >
              ...
              <Pressable
                ...
                onPress={() => {
                  ...
                  if (match) {
                    const roleIds =
                      'roles' in match ? match.roles : match.roleIds;
                    setPreviewScript({
                      name: match.name,
                      roleIds,
                    });
            ...
            {isWide ? (
              ...
              <ScrollView
                style={styles.scriptScrollFull}
                showsVerticalScrollIndicator={false}
              >
                <View style={styles.scriptList}>
                  {savedScripts.length > 0 && (
                    <>
                      <Text style={styles.scriptSectionLabel}>
                        Saved Scripts
                      </Text>
                      {savedScripts.map(s => (
                        <Pressable
                          key={s.id}
                          style={[
                            styles.scriptRowItem,
                            selectedScript === s.id && styles.scriptRowActive,
                          ]}
                        ...
                        >
                          ...
                          <Text
                            style={[
                              styles.scriptRowText,
                              selectedScript === s.id &&
                                styles.scriptRowTextActive,
                            ]}
                          >
                            {s.name}
                          </Text>
                  ...
                  {scripts.map(s => (
                    ...
                    >
                      ...
                      <Text
                        style={[
                          styles.scriptRowText,
                          selectedScript === s.id && styles.scriptRowTextActive,
                        ]}
                      >
                        {s.name}
                      </Text>
              ...
              <View style={styles.dropdown}>
                ...
                >
                  <Text style={styles.dropdownText}>
                    {[
                      ...scripts,
                      ...savedScripts,
                    ].find(s => s.id === selectedScript)?.name ||
                      'Select a script'}
                  </Text>
                ...
                <Modal
                  visible={showScriptDropdown}
                  transparent
                  animationType="fade"
                >
                  <Pressable
                    style={styles.modalOverlay}
                    onPress={() => setShowScriptDropdown(false)}
                  >
            ...
            {showFriends && friends.length > 0 && (
              <View style={styles.friendsSection}>
                ...
                <View style={styles.friendsGrid}>
                  {friends
                    .filter(f => !playerNames.includes(f.name))
                ...
                >
                  ...
                  <View style={styles.inputWrapper}>
                    <TextInput
                      ...
                      style={[
                        styles.playerInput,
                        friends.some(f => f.name === name) &&
                          styles.playerInputLocked,
                      ]}
                      value={name}
                      ...
                      onBlur={() =>
                        setTimeout(() => setAutoCompleteIdx(null), 200)
                      }
                      placeholder={`Player ${i + 1}`}
                      placeholderTextColor="#555"
                      returnKeyType={
                        i < playerNames.length - 1 ? 'next' : 'next'
                      }
                      onSubmitEditing={() => {
                        ...
                        {matches.map(f => (
                          <Pressable
                            ...
                            onPress={() => {
                              ...
                              const nextEmpty = playerNames.findIndex(
                                (n, idx) => idx > i && !n.trim(),
                              );
                              const next =
                                nextEmpty >= 0 ? nextEmpty : playerNames.length;
                              setTimeout(
