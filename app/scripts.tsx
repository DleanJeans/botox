...
export default function ScriptsScreen() {
  ...
  const [detailScript, setDetailScript] = useState<{ name: string; roleIds: string[] } | null>(null);
  const [showCreate, setShowCreate] = useState(false);

  const { scripts: importedScripts, saveScript, deleteScript } = useSavedScriptStore();
  const builtinScripts = getScripts();

  const refreshImported = useCallback(() => {
    useSavedScriptStore.getState().loadScripts();
  }, []);
  useEffect(() => { refreshImported(); }, []);
  ...
  const handleDownload = async (script: BotcScriptResult) => {
    ...
    try {
      ...
      const res = await fetch(`https://www.botcscripts.com/api/scripts/${script.pk}/`);
      const data = await res.json();
      const roleIds = extractRoleIds(data.content || script.content);
      saveScript(script.name, script.author || 'Unknown', script.version, roleIds);
      useSavedScriptStore.getState().loadScripts();
    } catch {
  ...
  const commonBrowseProps = {
    ...
    onPreview: (name: string, roleIds: string[]) => setDetailScript({ name, roleIds }),
  };
        ...
        {isWide ? (
          <View style={{ flex: 1, flexDirection: 'row', gap: 24 }}>
            ...
            <View style={{ flex: 1 }}>
              <ScriptSavedTab
                ...
                onScriptPress={(name, roleIds) => setDetailScript({ name, roleIds })}
          ...
          <>
            <View style={styles.tabBar}>
              ...
              >
                <Text style={[styles.tabText, viewTab === 'browse' && styles.tabTextActive]}>Browse</Text>
              </Pressable>
              <Pressable
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#2a2c30', marginBottom: 12,
  },
  title: { color: '#fff', fontSize: 22, fontWeight: '700', flex: 1 },
  createBtn: {
    backgroundColor: '#fcb93c', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8,
  },
  ...
  tabBar: {
    flexDirection: 'row', borderRadius: 10, backgroundColor: '#2f313a', padding: 4, marginBottom: 12,
  },
  tab: { flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 8 },
  tabActive: { backgroundColor: '#fcb93c' },
