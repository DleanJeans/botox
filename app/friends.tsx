    ...
    <SafeAreaView style={styles.container}>
      ...
      {friends.length > 0 && (
        <View style={styles.metaRow}>
          <Text style={styles.count}>{friends.length} friend{friends.length !== 1 ? 's' : ''}</Text>
          {friends.length > 5 && (
            <TextInput
            ...
            <View style={styles.friendRow}>
              <View style={styles.friendInfo}>
                ...
                <Text style={styles.friendMeta}>
                  Played {item.gameCount} time{item.gameCount !== 1 ? 's' : ''}
                  {item.lastPlayed ? ` · Last: ${new Date(item.lastPlayed).toLocaleDateString()}` : ' · Never played'}
                </Text>
              </View>
              <Pressable onPress={() => handleDelete(item.id, item.name)} style={styles.removeBtn}>
...
const styles = StyleSheet.create({
  ...
  count: { color: '#888', fontSize: 13, fontWeight: '500' },
  filterInput: {
    backgroundColor: '#2f313a', borderRadius: 6, paddingHorizontal: 10,
    paddingVertical: 4, color: '#fff', fontSize: 13, width: 120,
  },
  empty: {
  ...
  friendMeta: { color: '#888', fontSize: 12, marginTop: 3 },
  removeBtn: {
    width: 32, height: 32, borderRadius: 16, backgroundColor: '#3a1a1a',
